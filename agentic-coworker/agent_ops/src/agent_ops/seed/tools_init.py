...
def seed_all_tools(seed_dir:str):
    user="admin"
    try:
    ...
    with get_db_cm() as sess, driver.session() as gsess:
        ...
        tool_path = os.path.join( seed_dir, "tools", "seed_mcp_tools.json")
        app_key_path = os.path.join( seed_dir, "tools", "seed_app_keys.json")

        #load_initial_applications(sess, INITIAL_SERVICES_JSON_PATH)
        load_init_tools(etcd,sess, gsess, emb, llm, tool_path,user)
        load_app_keys(sess, app_key_path)
    close_graph_driver()
if __name__ == "__main__":
    seed_all_tools("/Users/jingnan.zhou/workspace/agentic-coworker/data/seed_data")
